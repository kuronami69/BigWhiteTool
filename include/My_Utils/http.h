...
int download_file(const char* url, const char* save_path) {
    ...
    int content_length = -1;  // 初始化为-1，表示未知
    ...
    while ((bytes_received = recv(sockfd, buffer, sizeof(buffer), 0)) > 0) {
        ...
        if (!header_end_found) {
            ...
            if (header_end) {
                header_end_found = 1;
                header_end += 4;  // 移动到响应头之后的内容

                // 获取Content-Length响应头的值
                char* content_length_str = strstr(buffer, "Content-Length: ");
                if (content_length_str) {
                    sscanf(content_length_str, "Content-Length: %d", &content_length);
                }

                int content_length_in_buffer = bytes_received - (header_end - buffer);
                fwrite(header_end, 1, content_length_in_buffer, file);
            } else {
                continue;
            }
        } else {
        ...
        // 如果已知内容长度并且已接收的数据达到或超过该长度，则停止接收
